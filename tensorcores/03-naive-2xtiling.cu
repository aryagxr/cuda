...
// 2x tiling:
// each warp is called 4 times
// so each warp computes 32*16 tile of the output
// 8 warps compute 64 * 64 of the output D
...
__global__ void tc_mma(half* A, half* B, float* C, float* D, int M, int N, int K) {
    ...
    //which row and col (32*16 tile) of the output
    int row_warp_offset = warp_row * 32;
    int col_warp_offset = warp_col * 16;

    // each block computes 32*32 tile of output D
    int blockM = blockIdx.y * 64;
    int blockN = blockIdx.x * 64;

    //8 warps compute 64*64 tile of output D
    //so A(64*16) * B(16*64) = D(64*64)
    ...
    // Initialize from C for each subtile
    for (int m = 0; m < 2; ++m) {
        for (int n = 0; n < 2; ++n) {
            for (int i = 0; i < 4; ++i) {
                // compute local row/col inside the warp tile (32x16)
                int local_row = m*16 + ((i < 2) ? groupID : (groupID + 8));
                int local_col = n*8 + ((tid_in_group * 2) + (i & 1));  
                int gm = blockM + row_warp_offset + local_row;
                int gn = blockN + col_warp_offset + local_col;
                dReg[m][n][i] = (gm < M && gn < N) ? C[gm * N + gn] : 0.0f;
            }
        }
    }
    ...
    for (int k_start = 0; k_start < K; k_start += 16){
        ...
        // per-warp fragments registers
        __half a_h[8];
        __half b_h[4];

        // for two m-positions and two n-positions
        for (int m = 0; m < 2; ++m) {            
            int mBase = m * 16;                  
            for (int n = 0; n < 2; ++n) {        
                int nBase = n * 8; 

                a_h[0] = A_shared[mWarp + mBase + groupID    ][ (tid_in_group*2)    ];
                a_h[1] = A_shared[mWarp + mBase + groupID    ][ (tid_in_group*2) + 1];
                a_h[2] = A_shared[mWarp + mBase + groupID + 8][ (tid_in_group*2)    ];
                a_h[3] = A_shared[mWarp + mBase + groupID + 8][ (tid_in_group*2) + 1];
                a_h[4] = A_shared[mWarp + mBase + groupID    ][ (tid_in_group*2) + 8];
                a_h[5] = A_shared[mWarp + mBase + groupID    ][ (tid_in_group*2) + 9];
                a_h[6] = A_shared[mWarp + mBase + groupID + 8][ (tid_in_group*2) + 8];
                a_h[7] = A_shared[mWarp + mBase + groupID + 8][ (tid_in_group*2) + 9];

                // Load B fragment for this (m,n)
                b_h[0] = B_shared[(tid_in_group*2) + 0][ nWarp + nBase + groupID ];
                b_h[1] = B_shared[(tid_in_group*2) + 1][ nWarp + nBase + groupID ];
                b_h[2] = B_shared[(tid_in_group*2) + 8][ nWarp + nBase + groupID ];
                b_h[3] = B_shared[(tid_in_group*2) + 9][ nWarp + nBase + groupID ];

                // pack halves into u32
                ...
                float tmpOut[4];
                mma_m16n8k16(A_packed, B_packed, &dReg[m][n][0], tmpOut);
                for (int i = 0; i < 4; ++i) dReg[m][n][i] = tmpOut[i];
            } 
        } 
...
int main() {
    ...
    //each thread block(8 warps together) computes a 64*64 tile of the output D
    ...
    // copy full M x N output matrix D back to host
    ...
    int showM = min(8, M);
    int showN = min(8, N);
    printf("D top-left %dx%d:\n", showM, showN);
    for (int i = 0; i < showM; ++i) {
        for (int j = 0; j < showN; ++j) {
            printf("%8.4f ", hD[i * N + j]);
        }
        printf("\n");
    }
